...
void main() {
  final MockCardTableRepository mockCardTableRepository = MockCardTableRepository();
  final CardTableMock cardTableMock = CardTableMock();

  setUpAll(() {
    injectionContainer.sl.registerSingleton<CardTableRepository>(mockCardTableRepository);
  });

  group("usecase DrawCard", () {
    test("should return an object of type CardModel", () async {
      when(mockCardTableRepository.getDrawCard(deckId: "3p40paa87x90", count: "2")).thenAnswer((_) async => right(CardModel.fromJson(cardTableMock.drawCardSuccess())));

      DrawCardImp drawCardImp = DrawCardImp(repository: injectionContainer.sl());
      final result = await drawCardImp(deckId: "3p40paa87x90", count: "2");

      expect(result.isRight(), true);
      result.fold((l) => null, (CardModel deckModel) => expect(deckModel, isA<CardModel>()));
    });

    test("should return an object of type CardTableRequestError", () async {
          var requestFail = cardTableMock.drawCardFail();
      when(mockCardTableRepository.getDrawCard(deckId: "3p40paa87x90", count: "2")).thenAnswer((_) async => left(CardTableRequestError(message: requestFail['error'] ?? "")));
