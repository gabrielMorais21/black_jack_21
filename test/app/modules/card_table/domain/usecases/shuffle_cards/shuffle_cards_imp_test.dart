  ...
  group("usecase DrawCard", () {
    test("should return an object of type CardModel", () async {
      when(mockCardTableRepository.getShuffleCards(
        deckCount: "6",
      )).thenAnswer((_) async => right(DeckModel.fromJson(cardTableMock.reshuffleTheCardsSuccess())));

      ShuffleCardsImp shuffleCardsImp = ShuffleCardsImp(repository: injectionContainer.sl());
    ...
    test("should return an object of type CardTableRequestError", () async {
      ...
      when(mockCardTableRepository.getShuffleCards(deckCount: "60")).thenAnswer((_) async => left(CardTableRequestError(message: requestFail['error'] ?? "")));
